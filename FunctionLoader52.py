...
class LuaByteCodeFunction52(object):
    ...
    def _DisassembleCode(self, current_pos):
        pc_offset = 0
        disasm = LuaDisasm(self.stream, self.instructionSize, self.codeSize, self.constants, self.isBE, LuaVersion.LuaVersion52)
    ...
    def _DecodeContant(self):
        ...
        for i in range(self.constantSize):
            ...
            if types == LUA_TNIL:
                ...
                int_val_buf = self.stream.read(8)
                int_val = 0
                bytes_readed = bytes_readed + 8
                if self.isBE == False:
                    int_val = unpack('<q', int_val_buf[0:8])[0]
                else:
                    int_val = unpack('>q', int_val_buf[0:8])[0]
    ...
    def ReadString(self):
        ...
        if self.isBE == False:
            if self.sizeOfSizeT == 4:
                bytesRead = 4
                stringLenBuf = self.stream.read(4)
                stringLen = unpack('<L', stringLenBuf[0:4])[0]
            else:
                bytesRead = 8
                stringLenBuf = self.stream.read(8)
                stringLen = unpack('<Q', stringLenBuf[0:8])[0]
        else:
            if self.sizeOfSizeT == 4:
                bytesRead = 4
                stringLenBuf = self.stream.read(4)
                ...
                stringLen = unpack('>Q', stringLenBuf[0:8])[0]
        if stringLen != 0:
            string = self.stream.read(stringLen - 1)
            self.stream.read(1)
            bytesRead += stringLen
        
        return bytesRead, string

    def _DecodeDebugNew(self):
        bytes_readed, source_str = self.ReadString()
    ...
    def _ParseLua52ByteCode(self):
        ...
        self.srcBegin = unpack('<L', start_line_buf[0:4])[0]
    ...
    def Parse(self):
        return self._ParseLua52ByteCode()
